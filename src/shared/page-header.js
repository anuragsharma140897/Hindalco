...
export default function PageHeader({ metaTitle, btnText, className, data = [], children, fileName, customSize, title, disbleExport, icon, titleClass, useConfirmBox, params, hideBreadCrumb, exportBtnText, href }) {
  ...
  return (
    <header className={cn('mb-6 @container xs:-mt-2 lg:mb-7', className)}>
      <div className="flex flex-col @lg:flex-row @lg:items-center @lg:justify-between">
        <div>
          <Title as="h2" className={`mb-2 text-[22px] lg:text-2xl 4xl:text-[26px] flex items-center`}> {metaObject?.title} {params ? <span className='flex items-center'><PiCaretRightBold /> {params}</span> : null} </Title>
          {
            hideBreadCrumb ? null : <Breadcrumb separator="" separatorVariant="circle" className="flex-wrap">
        ...
        <div className="mt-4 flex items-center gap-3 @lg:mt-0">
          ...
          {children || href ? <Button as="span" type="button" className="w-full @lg:w-auto cursor-pointer" onClick={() => handleFunctionCall()} >
