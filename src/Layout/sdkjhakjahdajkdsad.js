...
// Function to extract allowed permissions
const getAllowedPermissions = (role) => {
  const permissions = role?.permission?.reduce((acc, item) => {
    acc.push(item.value.toLowerCase());
    if (item.child && item.child.length > 0) {
      item.child.forEach((child) => {
        acc.push(child.value.toLowerCase());
      });
    }
    return acc;
  }, []);
...
// Extract allowed permissions from the authenticated user's role
const allowedPermissions = getAllowedPermissions(getAuthenticatedUser()?.roleId);
...
const filteredMenuItems = menuItems.map((menuItem) => {
  if (menuItem.dropdownItems) {
    const filteredDropdownItems = menuItem?.dropdownItems.filter((dropdownItem) =>
      allowedPermissions?.includes(dropdownItem.name.toLowerCase())
    );

    // If no dropdown items are allowed, exclude the parent menu item as well
    if (filteredDropdownItems.length === 0) {
      return null;
    }

    return { ...menuItem, dropdownItems: filteredDropdownItems };
  } else if (allowedPermissions?.includes(menuItem.name.toLowerCase())) {
    return menuItem;
  }
