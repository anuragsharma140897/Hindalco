...
export default function AddUserMaster({ row, closeModal, ApiHit }) {
    ...
    const { openAlert, AlertComponent } = useAlertController();
    ...
    const handleSubmit = (e) => {
        ...
        //             if (result) {
        //                 openAlert("success", "Success", "User Added Successfully")
        //                 if (ApiHit) { ApiHit() }
        //             }
        //         })
        //     }
            ...
            <form onSubmit={handleSubmit}>
                <div className="space-y-5 lg:space-y-6">
                    <div className='grid grid-cols-2 gap-4'>
                        ...
                        <SearchableSelect name="gender" label="Gender" api={searchGeneral} serverKey={'fieldName'} serverValue={'gender'} getFieldName={'value'} value={reduxUser?.apiJson?.roleName} error={errors} reduxState={reduxUser?.apiJson} setAction={setUserApiJson} />
                        <SearchableSelect name="roleName" label="Role" api={searchRole} getFieldName={'roleName'} value={reduxUser?.apiJson?.roleName} error={errors} reduxState={reduxUser?.apiJson} setAction={setUserApiJson} onChange={handleCustomChange} />
