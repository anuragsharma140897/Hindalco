...
import {  removeBuildingFromZone } from "../../../../Constant/Api/Api";
...
export const getBuildingZoneMasterColumns = ({ zoneid }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex">
          <div onClick={() => DeleteItem(row._id, zoneid)} className="border px-2 py-1 rounded cursor-pointer" ><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
export const DeleteItem = (_id, zoneid) => {
